...
static const int original_message_expansion[] =  {
	32,  1,  2,  3,  4,  5,
	 4,  5,  6,  7,  8,  9,
	 8,  9, 10, 11, 12, 13,
	12, 13, 14, 15, 16, 17,
...
void des_set_salt(uint16_t salt) {
	int i;

	// Reset the working array to the standard baseline before every encryption
	for (i = 0; i < 48; ++i) {
		message_expansion[i] = original_message_expansion[i];
	}

	// Apply the 16-bit salt swaps
	for (i = 0; i < 16; ++i) {
		// If the i-th bit of the salt is 1, perform the wire swap
		if ((salt >> i) & 1) {
			int temp = message_expansion[i];
			message_expansion[i] = message_expansion[i + 24];
			message_expansion[i + 24] = temp;
		}
	}
}